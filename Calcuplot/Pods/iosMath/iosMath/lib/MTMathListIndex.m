...
#define INDEQUAL(a,b) (a.cat==b.cat&&(a.cat!=kMTSubIndexTypeMatrix||(a.x==b.x&&a.y==b.y)))
...
- (MTMathListSubIndexType) create:(MTMathListSubtypeEnum) cate
{
    MTMathListSubIndexType rets;
    rets.cat = cate;
    rets.x = 0;
    rets.y = 0;
    return rets;
}
- (MTMathListSubIndexType) create:(MTMathListSubtypeEnum) cate ind:(int)xss
{
    MTMathListSubIndexType rets;
    rets.cat = cate;
    rets.x = xss;
...
- (MTMathListIndex *)levelUpWithSubIndex:(MTMathListIndex *)subIndex fulltype:(MTMathListSubIndexType)type
{
    if (self.subIndexType.cat == kMTSubIndexTypeNone) {
        return [MTMathListIndex indexAtLocation:self.atomIndex withSubIndex:subIndex type:type];
    }
    // we have to recurse
    return [MTMathListIndex indexAtLocation:self.atomIndex withSubIndex:[self.subIndex levelUpWithSubIndex:subIndex fulltype:type] type:self.subIndexType];
}

- (MTMathListIndex *)levelUpWithSubIndex:(MTMathListIndex *)subIndex type:(MTMathListSubtypeEnum)type
{
    if (self.subIndexType.cat == kMTSubIndexTypeNone) {
        return [MTMathListIndex indexAtLocation:self.atomIndex withSubIndex:subIndex type:[[MTMathListIndex alloc] create:type]];
    }
    // we have to recurse
...
- (MTMathListIndex *)next
{
    if (self.subIndexType.cat == kMTSubIndexTypeNone) {
        return [MTMathListIndex level0Index:self.atomIndex + 1];
    } else if (self.subIndexType.cat == kMTSubIndexTypeNucleus) {
...
- (BOOL)hasSubIndexOfType:(MTMathListSubtypeEnum) subIndexType
{
    if (self.subIndexType.cat == subIndexType) {
        return true;
    } else {
        return [self.subIndex hasSubIndexOfType:subIndexType];
    }
}
- (BOOL)hasSubIndexOfFullType:(MTMathListSubIndexType)subIndexType
{
    if (INDEQUAL(self.subIndexType,subIndexType)) {
        return true;
    } else {
        return [self.subIndex hasSubIndexOfFullType:subIndexType];
    }
}
...
{
    if (!INDEQUAL(self.subIndexType,other.subIndexType)) {
        return false;
    } else if (self.subIndexType.cat == kMTSubIndexTypeNone) {
...
{
    if (self.subIndex) {
        return [NSString stringWithFormat:@"[%lu, %d:%@]", (unsigned long)self.atomIndex, self.subIndexType.cat, self.subIndex];
    }
...
{
    if (self.atomIndex != index.atomIndex || !INDEQUAL(self.subIndexType,index.subIndexType)) {
        return NO;
    }
    if (self.subIndex) {
...
- (NSUInteger) hash
{
    printf("oh god oh fuck\n");
    const int prime = 31;
    NSUInteger hash = self.atomIndex;
    hash = hash * prime + self.subIndexType.cat;
    hash = hash * prime;
    if (self.subIndexType.cat == kMTSubIndexTypeMatrix) {
        hash = hash + self.subIndexType.x*5 + self.subIndexType.y;
    }
...
{
    if (self.start.subIndexType.cat != kMTSubIndexTypeNone) {
