...
{
//    if (_keyboard==nil) {
//        _keyboard = [MTMathKeyboardRootView sharedInstance];
//    }
    if (![self isFirstResponder]) {
...
- (void) insertionPointChanged
{
    ...
    if (atom.type == kMTMathAtomPlaceholder) {
        ...
        if (_insertionIndex.finalSubIndexType.cat == kMTSubIndexTypeNucleus) {
...
- (void) handleExponentButton
{
    if ([_insertionIndex hasSubIndexOfType:kMTSubIndexTypeSuperscript]) {
        ...
        if (!_insertionIndex.isAtBeginningOfLine) {
            ...
            } else if (_insertionIndex.finalSubIndexType.cat == kMTSubIndexTypeNucleus) {
...
- (void) handleSubscriptButton
{
    if ([_insertionIndex hasSubIndexOfType:kMTSubIndexTypeSubscript]) {
        ...
        _insertionIndex = [self getIndexAfterSpecialStructure:_insertionIndex type:[[MTMathListIndex alloc] create:kMTSubIndexTypeSubscript]];
    } else {
...
- (void) handleSubSuper
{
    ...
    [self.mathList insertAtom:[self atomForCharacter:'d'] atListIndex:_insertionIndex];
...
- (void) handleDeriv
{
    ...
    [numerator addAtom:[MTMathAtomFactory atomForLatexSymbolName:@"partial"]];
    MTFraction *frac = [MTFraction new];
    frac.denominator = [MTMathList new];
    [frac.denominator addAtom:[MTMathAtomFactory atomForLatexSymbolName:@"partial"]];
    ...
    [self.mathList insertAtom:frac atListIndex:_insertionIndex];
    // update the insertion index to go the denominator
    _insertionIndex = [_insertionIndex levelUpWithSubIndex:[MTMathListIndex level0Index:0] type:kMTSubIndexTypeDenominator].next;

}


- (void) handleTwoVector
{
...
//    MTMathList* bottom = [MTMathList new];
//    [top addAtom:[MTMathAtomFactory placeholder]];
//    [bottom addAtom:[MTMathAtomFactory placeholder]];
//
//    [self.mathList insertAtom:
//    [MTMathAtomFactory tableWithEnvironment:@"bmatrix" rows:@[@[top],@[bottom]] error:NULL]
//    atListIndex:_insertionIndex];
}
- (void) handleThreeVector
{
//    [self.mathList insertAtom:[self atomForCharacter:'['] atListIndex:_insertionIndex];
//    _insertionIndex = _insertionIndex.next;
//    MTMathAtom* atom = [self.mathList atomAtListIndex:_insertionIndex.previous];
...
//    _insertionIndex = [_insertionIndex.previous levelUpWithSubIndex:[MTMathListIndex level0Index:0] type:kMTSubIndexTypeSubscript];
    ...
    MTMathAtom* atom = [self atomForCharacter:'['];
    atom.superScript = [MTMathList new];
    MTMathAtom* satom = [self atomForCharacter:'|'];
    [atom.superScript addAtom:satom];

    satom.subScript = [MTMathList new];
    [satom.subScript addAtom:[MTMathAtomFactory placeholder]];
    satom.nucleus = @"";
    
    atom.subScript = [MTMathList new];
    satom = [self atomForCharacter:'|'];
    [atom.subScript addAtom:satom];
    satom.subScript = [MTMathList new];
    [satom.subScript addAtom:[MTMathAtomFactory placeholder]];
    satom.superScript = [MTMathList new];
    [satom.superScript addAtom:[MTMathAtomFactory placeholder]];
    satom.nucleus = @"";
...
//    _insertionIndex = _insertionIndex.next;
//    _insertionIndex = [_insertionIndex levelUpWithSubIndex:[MTMathListIndex level0Index:atom.subScript.atoms.count] type:kMTSubIndexTypeSubscript];
...
//    MTMathList* top    = [MTMathList new];
//    MTMathList* middle = [MTMathList new];
//    MTMathList* bottom = [MTMathList new];
//    [top addAtom:[MTMathAtomFactory placeholder]];
//    [middle addAtom:[MTMathAtomFactory placeholder]];
//    [bottom addAtom:[MTMathAtomFactory placeholder]];
//
//    [self.mathList insertAtom:
//    [MTMathAtomFactory tableWithEnvironment:@"bmatrix" rows:@[@[top],@[middle],@[bottom]] error:NULL]
//    atListIndex:_insertionIndex];
//
//    _insertionIndex = [_insertionIndex.previous levelUpWithSubIndex:[MTMathListIndex level0Index:0] fulltype:[[MTMathListIndex alloc]create:kMTSubIndexTypeMatrix ind:0]];
...
- (MTMathListIndex *) getIndexAfterSpecialStructure:(MTMathListIndex *) index type:(MTMathListSubIndexType)type
{
    ...
    while ([nextIndex hasSubIndexOfFullType:type]){
...
- (MTMathListIndex *) getOutOfRadical:(MTMathListIndex *)index {
    if ([index hasSubIndexOfType:kMTSubIndexTypeDegree]) {
        index = [self getIndexAfterSpecialStructure:index type:[[MTMathListIndex alloc] create:kMTSubIndexTypeDegree]];
    }
    if ([index hasSubIndexOfType:kMTSubIndexTypeRadicand]) {
        index = [self getIndexAfterSpecialStructure:index type:[[MTMathListIndex alloc] create:kMTSubIndexTypeRadicand]];
    }
    return index;
}
...
- (void) insertText:(NSString*) str
{
    ...
    NSDictionary *parkerfix = @{@"sin⁻¹":@"arcsin",@"cos⁻¹":@"arccos",@"tan⁻¹":@"arctan",@"𝝅":@"pi"};
    if (parkerfix[str]) str = parkerfix[str];
    ...
    if (_insertionIndex.subIndexType.cat == kMTSubIndexTypeDenominator) {
    ...
    if (ch == '^') {
        ...
        [self removePlaceholderIfPresent];
        [self insertMagnitude];
    } else if ([str isEqualToString:@"subsuper"]) {
        [self removePlaceholderIfPresent];
        [self handleSubSuper];
    } else if ([str isEqualToString:@"twovector"]) {
        [self removePlaceholderIfPresent];
        [self handleTwoVector];
    } else if ([str isEqualToString:@"threevector"]) {
        [self removePlaceholderIfPresent];
        [self handleThreeVector];
    } else if ([str isEqualToString:@"deriv"]) {
        [self removePlaceholderIfPresent];
        [self handleDeriv];
    } else if (atom) {
...
//- (BOOL)parkerfix:(NSString *)string {
//    NSArray *trigFunctions = @[@"sin",  @"cos",  @"tan",  @"sec",  @"csc",  @"cot",
...
- (void) insertMagnitude
{
    NSString* ch = @"|";
    MTMathAtom* atom = [MTMathAtomFactory atomForLatexSymbolName:ch];
    [self.mathList insertAtom:atom atListIndex:_insertionIndex];
    _insertionIndex = _insertionIndex.next;
    atom = [MTMathAtomFactory atomForLatexSymbolName:ch];
    [self.mathList insertAtom:atom atListIndex:_insertionIndex];
    // Don't go to the next insertion index, to start inserting before the second absolute value
}
...
{    
    ...
    if (self.hasText && prevIndex) {
        ...
        if (prevIndex.finalSubIndexType.cat == kMTSubIndexTypeNucleus) {
        ...
        if (_insertionIndex.isAtBeginningOfLine && _insertionIndex.subIndexType.cat != kMTSubIndexTypeNone) {
