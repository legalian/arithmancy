...
{
    CGFloat offset;
    NSAssert(index.subIndexType.cat == kMTSubIndexTypeNone, @"Index in a CTLineDisplay cannot have sub indexes.");
...
- (void)highlightCharacterAtIndex:(MTMathListIndex *)index color:(UIColor *)color
{
    ...
    assert(index.subIndexType.cat == kMTSubIndexTypeNone || index.subIndexType.cat == kMTSubIndexTypeNucleus);
    if (index.subIndexType.cat == kMTSubIndexTypeNucleus) {
...
- (MTMathListIndex *)closestIndexToPoint:(CGPoint)point
{
    ...
    if (point.x < self.position.x - kPixelDelta) {
        ...
        if (numeratorDistance < denominatorDistance) {
            return [MTMathListIndex indexAtLocation:self.range.location withSubIndex:[self.numerator closestIndexToPoint:point] type:[[MTMathListIndex alloc]create:kMTSubIndexTypeNumerator]];
        } else {
            return [MTMathListIndex indexAtLocation:self.range.location withSubIndex:[self.denominator closestIndexToPoint:point] type:[[MTMathListIndex alloc]create:kMTSubIndexTypeDenominator]];
        }
    }
}
...
- (CGPoint)caretPositionForIndex:(MTMathListIndex *)index
{
    assert(index.subIndexType.cat == kMTSubIndexTypeNone);
...
- (MTMathListDisplay*) subAtomForIndexType:(MTMathListSubIndexType) type
{
    switch (type.cat) {
        ...
        case kMTSubIndexTypeMatrix:
        case kMTSubIndexTypeNone:
            NSAssert(false, @"Not a fraction subtype %d", type.cat);
            return nil;
    }
}
...
//#pragma mark - MTMatrixDisplay

//@interface MTMatrixDisplay (Editing)
//
//// Find the index in the mathlist before which a new character should be inserted. Returns nil if it cannot find the index.
//- (MTMathListIndex*) closestIndexToPoint:(CGPoint) point;
//
...
- (MTMathListIndex *)closestIndexToPoint:(CGPoint)point
{
    ...
    if (point.x < self.position.x - kPixelDelta) {
        ...
        if (degreeDistance < radicandDistance) {
            return [MTMathListIndex indexAtLocation:self.range.location withSubIndex:[self.degree closestIndexToPoint:point] type:[[MTMathListIndex alloc]create:kMTSubIndexTypeDegree]];
        } else {
            return [MTMathListIndex indexAtLocation:self.range.location withSubIndex:[self.radicand closestIndexToPoint:point] type:[[MTMathListIndex alloc]create:kMTSubIndexTypeRadicand]];
        }
...
- (MTMathListDisplay*) subAtomForIndexType:(MTMathListSubIndexType) type
{
    switch (type.cat) {
        ...
        case kMTSubIndexTypeMatrix:
        case kMTSubIndexTypeNone:
            NSAssert(false, @"Not a radical subtype %d", type.cat);
            return nil;
    }
}
...
//    NSLog(@"%@",NSStringFromClass([atomWithPoint class]));
...
//        NSAssert(false,"googaga");
        MTMathListSubIndexType type;
        switch(closestLine.type) {
            case kMTLinePositionRegular: {
                type = [[MTMathListIndex alloc]create:kMTSubIndexTypeMatrix ind:wnk];
                break;
            }
            case kMTLinePositionSubscript: {
                type = [[MTMathListIndex alloc]create:kMTSubIndexTypeSubscript];
                break;
            }
            case kMTLinePositionSuperscript: {
                type = [[MTMathListIndex alloc]create:kMTSubIndexTypeSuperscript];
                break;
            }
        }
...
//        NSAssert(closestLine.index != NSNotFound, @"Index is not set for a subscript/superscript");
        ...
        if (index.atomIndex == NSMaxRange(atomWithPoint.range)) {
            return [MTMathListIndex indexAtLocation:index.atomIndex - 1 withSubIndex:[MTMathListIndex level0Index:1] type:[[MTMathListIndex alloc]create:kMTSubIndexTypeNucleus]];
        }
    }
    return index;
}
...
{
    // Inside the range
    if (index.subIndexType.cat == kMTSubIndexTypeSuperscript || index.subIndexType.cat == kMTSubIndexTypeSubscript) {
        for (MTDisplay* atom in self.subDisplays) {
            if ([atom isKindOfClass:[MTMathListDisplay class]]) {
                ...
                if (index.atomIndex == lineAtom.index) {
                    ...
                    if ((lineAtom.type == kMTLinePositionSubscript && index.subIndexType.cat == kMTSubIndexTypeSubscript)
                        || (lineAtom.type == kMTLinePositionSuperscript && index.subIndexType.cat == kMTSubIndexTypeSuperscript)) {
                        return lineAtom;
                    }
                }
            }
...
- (CGPoint)caretPositionForIndex:(MTMathListIndex *)index
{
    ...
    if (index.atomIndex == NSMaxRange(self.range)) {
        ...
        } else if (index.subIndexType.cat == kMTSubIndexTypeNone) {
...
- (void)highlightCharacterAtIndex:(MTMathListIndex *)index color:(UIColor *)color
{
    ...
    if (NSLocationInRange(index.atomIndex, self.range)) {
        MTDisplay* atom = [self subAtomForIndex:index];
        if (index.subIndexType.cat == kMTSubIndexTypeNucleus || index.subIndexType.cat == kMTSubIndexTypeNone) {
